...
export const useInvestmentSummary = (id) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			const walletBalancesnapshot = await get(walletBalancedbRef);
			const investedAmountsnapshot = await get(investedAmountbRef);

			if (walletBalancesnapshot.exists()) {
				const targetObject = walletBalancesnapshot.val();
				setWalletBalance(targetObject.amount);
			} else {
				alert("you haven't deposited yet");
			}
			...
			if (investedAmountsnapshot.exists()) {
				...
				alert("You haven't invested yet");
			}
		};
		fetchData();
	}, []);

	const updatedWalletBalance = walletBalance - investedAmount;
